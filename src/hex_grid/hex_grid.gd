		...
		match usable_card.usable_type:
			UsableCardData.UsableType.BACK_TO_HAND:
				...
				if _hex:
					...
					GameManager.card_holder.next_card_add_position = _hex.global_position
					var added: CardHudBase = GameManager.card_holder.add_card(returned_card)
					if not added: GameManager.card_holder.queue_add_card(returned_card)
					clear_hex_at(pos)
